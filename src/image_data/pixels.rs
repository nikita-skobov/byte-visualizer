...
impl Pixel {
    ...
    pub fn get_blended_pixel_tuple(
        pix1: (u8, u8, u8, u8),
        pix2: (u8, u8, u8, u8),
    ) -> (u8, u8, u8, u8) {
        if pix2.3 == 255 {
        ...
        let red = (((alpha_inverse * pix1_alpha_f * pix1.0 as f32)
            + (pix2_alpha_f * pix2.0 as f32))
            / output_alpha) as u8;
        let green = (((alpha_inverse * pix1_alpha_f * pix1.1 as f32)
            + (pix2_alpha_f * pix2.1 as f32))
            / output_alpha) as u8;
        let blue = (((alpha_inverse * pix1_alpha_f * pix1.2 as f32)
            + (pix2_alpha_f * pix2.2 as f32))
            / output_alpha) as u8;
    ...
    pub fn get_blended_pixel(bottom: Pixel, top: Pixel) -> Pixel {
        ...
        let red = (((alpha_inverse * bottom_alpha_f * bottom.red as f32)
            + (top_alpha_f * top.red as f32))
            / output_alpha) as u8;
        let green = (((alpha_inverse * bottom_alpha_f * bottom.green as f32)
            + (top_alpha_f * top.green as f32))
            / output_alpha) as u8;
        let blue = (((alpha_inverse * bottom_alpha_f * bottom.blue as f32)
            + (top_alpha_f * top.blue as f32))
            / output_alpha) as u8;
        let alpha = (output_alpha * 255.0) as u8;
