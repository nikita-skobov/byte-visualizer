...
fn get_output_name(file_path: &str) -> String {
...
fn get_file_as_binary(file_path: &str) -> Vec<u8> {
...
fn get_dimensions_from_len(len: f64) -> (u32, u32) {
  let sqrt = len.sqrt();
...
fn fill_image_data(
  ...
  for i in 0..image_data.len() {
    ...
    if iterator % values_per_pixel == 0 {
      let red_value = if index < binary_data.len() { binary_data[index] } else { 0 };
...
fn main() {
  let (file_path, output_name) = get_args();
  let binary_data = get_file_as_binary(&file_path);
  let (width, height) = get_dimensions_from_len(binary_data.len() as f64);
  ...
  println!("data len: {}, width: {}, height: {}, width*height: {}", binary_data.len(), width, height, width * height);
}
